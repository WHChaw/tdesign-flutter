...
class GMSwiperPagination extends SwiperPlugin {
  const GMSwiperPagination(
      {this.alignment,
      this.key,
      this.margin = const EdgeInsets.all(10.0),
      this.builder = GMSwiperPagination.dots});

  /// 圆点样式
  static const SwiperPlugin dots = GMSwiperDotsPagination();

  /// 圆角矩形 + 圆点样式 默认宽度20，高度6
  static const SwiperPlugin dotsBar =
      GMSwiperDotsPagination(roundedRectangleWidth: 20);

  /// 数字样式
  static const SwiperPlugin fraction = GMFractionPagination();

  /// 箭头样式
  static const SwiperPlugin controls = GMSwiperArrowPagination();
...
class GMSwiperDotsPagination extends SwiperPlugin {
  ...
  Widget build(BuildContext context, SwiperPluginConfig config) {
    if (config.itemCount > 20) {
      print('warning: The itemCount is too big, '
          'we suggest use GMFractionPaginationBuilder');
    }
    ...
    var color = this.color ??
        (config.outer
            ...
            : GMTheme.of(context).whiteColor1.withOpacity(0.55));
...
class GMFractionPagination extends SwiperPlugin {
  // container宽度
  final double? width;
...
class GMSwiperArrowPagination extends SwiperPlugin {
      ...
      Visibility(
        ...
        child: GestureDetector(
          child: CircleAvatar(
            radius: radius ?? 10.0,
            backgroundColor:
                backgroundColor ?? GMTheme.of(context).fontGyColor3,
            child: backArrow ??
                const Icon(
