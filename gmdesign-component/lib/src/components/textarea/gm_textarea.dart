...
import '../../theme/gm_font_family.dart';
import '../../theme/gm_fonts.dart';
import '../../theme/gm_radius.dart';
import '../../theme/gm_spacers.dart';
import '../../theme/gm_theme.dart';
import '../input/input_view.dart';
import '../input/gm_input.dart';
import '../text/gm_text.dart';

enum GMTextareaLayout { vertical, horizontal }

/// 用于多行文本信息输入
class GMTextarea extends StatefulWidget {
  const GMTextarea({
    Key? key,
    this.width,
    this.textStyle,
    ...
    this.layout = GMTextareaLayout.horizontal,
    this.autosize,
    this.labelIcon,
    this.labelWidth,
    this.margin,
  ...
  @override
  _GMTextareaState createState() => _GMTextareaState();
}

class _GMTextareaState extends State<GMTextarea> {
  ...
  Widget _getLabelView(BuildContext context) {
    var padding = _getInputPadding(context);
    var isHorizontal = widget.layout == GMTextareaLayout.horizontal;
    var fontSize = isHorizontal ? GMTheme.of(context).fontBodyLarge?.size : GMTheme.of(context).fontBodyMedium?.size;
    if ((widget.label == null || widget.label == '') && widget.labelIcon == null && widget.labelWidget == null) {
      return const SizedBox.shrink();
    }
    return Container(
      width: widget.labelWidth,
      padding: isHorizontal ? EdgeInsets.only(right: padding) : EdgeInsets.only(bottom: GMTheme.of(context).spacer8),
      child: Row(
        ...
        children: [
          ...
          widget.label != null && widget.label != ''
              ? Flexible(
                  child: Padding(
                    padding: EdgeInsets.only(left: widget.labelIcon != null ? GMTheme.of(context).spacer4 : 0),
                    child: GMText(
                      widget.label!,
          ...
          widget.required == true
              ? Padding(
                  padding: EdgeInsets.only(left: GMTheme.of(context).spacer4),
                  child: GMText(
                    '*',
                    style: TextStyle(color: GMTheme.of(context).errorColor6, fontSize: fontSize, height: 1.3),
                  ),
                )
              : const SizedBox.shrink(),
        ],
  ...
  Widget _getInputView(BuildContext context) {
    return SingleChildScrollView(
      child: ConstrainedBox(
        ...
        child: GMInputView(
          textStyle: widget.textStyle ?? TextStyle(color: GMTheme.of(context).fontGyColor1),
          ...
          hintTextStyle: widget.hintTextStyle ??
              TextStyle(
                  color: widget.readOnly == true ? GMTheme.of(context).fontGyColor4 : GMTheme.of(context).fontGyColor3),
  ...
  Widget _getIndicatorView(BuildContext context) {
              ...
              return Opacity(
                ...
                child: GMText(
                  ...
                  style: TextStyle(
                    ...
                    color: widget.additionInfoColor ?? GMTheme.of(context).fontGyColor3,
                  ),
                ),
              );
            },
    ...
    if (showIndicator) {
      widgetList.add(GMText(
        '${widget.controller?.text.length ?? 0}/${widget.maxLength}',
        style: TextStyle(fontSize: GMTheme.of(context).fontBodySmall?.size, color: GMTheme.of(context).fontGyColor3),
      ));
    }
    return Visibility(
  ...
  double _getInputPadding(BuildContext context) {
    switch (widget.size) {
      case GMInputSize.small:
        return GMTheme.of(context).spacer12;
      case GMInputSize.large:
        return GMTheme.of(context).spacer16;
      default:
        return GMTheme.of(context).spacer16;
    }
  }
