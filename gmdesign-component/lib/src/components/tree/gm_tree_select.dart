...
typedef GMTreeSelectChangeEvent = void Function(List<dynamic>, int level);

class GMSelectOption {
  GMSelectOption(
  ...
  /// 子选项
  List<GMSelectOption> children;
}

enum GMTreeSelectStyle {
  normal,
  outline,
}

class GMTreeSelect extends StatefulWidget {
  const GMTreeSelect(
      {Key? key,
      this.options = const [],
      ...
      this.onChange,
      this.multiple = false,
      this.style = GMTreeSelectStyle.normal,
      this.height = 336})
      : super(key: key);

  /// 展示的选项列表
  final List<GMSelectOption> options;

  /// 初始值，对应options中的value值
  final List<dynamic> defaultValue;

  /// 选中值发生变化
  final GMTreeSelectChangeEvent? onChange;
  ...
  /// 一级菜单样式
  final GMTreeSelectStyle style;

  @override
  State<GMTreeSelect> createState() => _GMTreeSelectState();
}

class _GMTreeSelectState extends State<GMTreeSelect> {
  ...
  List<GMSelectOption> get firstOptions => widget.options;
  List<GMSelectOption> get secondOptions => maxLevel() <= 1 || values.isEmpty
      ? []
      : firstOptions
          .firstWhere((opt) => opt.value == firstValue,
              orElse: () => GMSelectOption(value: -1, label: '', children: []))
          .children;
  List<GMSelectOption> get thirdOptions => maxLevel() <= 2 || currentLevel < 3
      ? []
      : secondOptions
      ...
      children: [
        SizedBox(
          ...
          child: GMSideBar(
            ...
            value: firstValue,
            style: widget.style == GMTreeSelectStyle.outline
                ? GMSideBarStyle.outline
                : GMSideBarStyle.normal,
