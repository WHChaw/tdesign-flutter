...
/// 计时组件
class GMTimeCounter extends StatefulWidget {
  const GMTimeCounter({
    Key? key,
    this.autoStart = true,
    ...
    this.size = GMTimeCounterSize.medium,
    this.splitWithUnit = false,
    this.theme = GMTimeCounterTheme.defaultTheme,
    required this.time,
    this.style,
    this.onChange,
    this.onFinish,
    this.direction = GMTimeCounterDirection.down,
    this.controller,
  }) : super(key: key);
  ...
  /// 尺寸
  final GMTimeCounterSize size;

  /// 使用时间单位分割
  final bool splitWithUnit;

  /// 风格
  final GMTimeCounterTheme theme;

  /// 必需；计时时长，单位毫秒
  final int time;

  /// 自定义样式，有则优先用它，没有则根据size和theme选取
  final GMTimeCounterStyle? style;
  ...
  /// 计时方向，默认倒计时
  final GMTimeCounterDirection direction;

  /// 控制器，可控制开始/暂停/继续/重置
  final GMTimeCounterController? controller;

  @override
  _GMTimeCounterState createState() => _GMTimeCounterState();
}

class _GMTimeCounterState extends State<GMTimeCounter> with SingleTickerProviderStateMixin {
  late GMTimeCounterStyle _style;
  ...
  @override
  void didUpdateWidget(GMTimeCounter oldWidget) {
  ...
  void startTimer() {
    ...
    _ticker ??= createTicker((Duration elapsed) {
      if ((widget.direction == GMTimeCounterDirection.down && _time > 0) ||
          widget.direction == GMTimeCounterDirection.up && _time < _maxTime) {
        setState(() {
          if (widget.direction == GMTimeCounterDirection.down) {
  ...
  void _onControllerChanged() {
    switch (widget.controller?.value) {
      case GMTimeCounterStatus.start:
        startTimer();
        break;
      case GMTimeCounterStatus.pause:
        pauseTimer();
        break;
      case GMTimeCounterStatus.resume:
        resumeTimer();
        break;
      case GMTimeCounterStatus.reset:
