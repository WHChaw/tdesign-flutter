...
enum GMTableColFixed { left, right, none }

enum GMTableColAlign { left, center, right }

typedef OnCellTap = void Function(int rowIndex, dynamic row, GMTableCol col);
...
class GMTableCol {
  GMTableCol({
    ...
    this.colKey,
    this.width,
    this.fixed = GMTableColFixed.none,
    this.ellipsis,
    this.ellipsisTitle,
    this.cellBuilder,
    this.align = GMTableColAlign.left,
    this.sortable = false,
  });
...
class GMTable extends StatefulWidget {
  ...
  /// 列配置
  final List<GMTableCol> columns;

  /// 数据源
  final List<dynamic>? data;

  /// 空表格呈现样式
  final GMTableEmpty? empty;
  ...
  @override
  State<GMTable> createState() => GMTableState();
}

class GMTableState extends State<GMTable> {
  bool? _sortable;
  String? _sortKey;
  final _scrollController = ScrollController();

  /// 获取单元格对齐方式
  Alignment _getVerticalAlign(GMTableColAlign x) {
    var xPos = 0.0;
    switch (x) {
      case GMTableColAlign.left:
        xPos = -1;
        break;
      case GMTableColAlign.center:
        xPos = 0;
        break;
      case GMTableColAlign.right:
        xPos = 1;
        break;
    }
  ...
  /// 过滤列配置
  List<GMTableCol> _getCol(GMTableColFixed fixed) {
    return widget.columns.where((col) => col.fixed == fixed).toList();
  }

  /// 生成表头
  Widget _getTableHeader(BuildContext context) {
    var fixedLeftCol = _getCol(GMTableColFixed.left);
    var fixedNonCol = _getCol(GMTableColFixed.none);
    var fixedRightCol = _getCol(GMTableColFixed.right);
    var start = 0;
      ...
      return Align(
        ...
        child: Padding(
          ...
          child: widget.loadingWidget ??
              const GMLoading(size: GMLoadingSize.large),
        ),
      );
    }
  ...
  /// 获取单元格
  Widget _getCell(GMTableCol col, bool isHeader, dynamic data, int index,
      bool fixedBorder) {
    ...
    if (fixedBorder && col.fixed == GMTableColFixed.left) {
      rightBorder = doubleBorder;
    }
    if (fixedBorder && col.fixed == GMTableColFixed.right) {
  ...
  /// 获取单元格内容
  Widget _getCellText(GMTableCol col, String title, bool ellipsis,
      bool isHeader, bool sortable, int index) {
    var overflow = ellipsis ? TextOverflow.ellipsis : TextOverflow.visible;
    var titleWidget = GMText(title,
        maxLines: 1,
        overflow: overflow,
        style: TextStyle(
          color: isHeader
              ? GMTheme.of(context).fontGyColor3
    ...
    // 表头（需考虑排序模式）
    if (isHeader) {
      var selectColor = GMTheme.of(context).brandNormalColor;
      var unSelectColor = GMTheme.of(context).fontGyColor3;
      return Row(
      ...
      child: Align(
        ...
        child: Padding(
          ...
          child: GMEmpty(
            image: Visibility(
              visible: false,
              child: GMImage(assetUrl: widget.empty?.assetUrl ?? ''),
            ),
  ...
  List<Widget> _getVerticalCell(
      List<GMTableCol> cols, List<List<String>> titles, double cellWidth) {
    var rows = <Widget>[];
  ...
  /// 获取每列单元格内容
  List<List<String>> _getCellsText(List<GMTableCol> cols) {
  ...
  Widget build(BuildContext context) {
    ...
    var width = widget.width ?? MediaQuery.of(context).size.width;
    var fixedCols = [
      ..._getCol(GMTableColFixed.left),
      ..._getCol(GMTableColFixed.right)
    ];
