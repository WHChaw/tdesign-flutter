...
///
///
class GMCheckbox extends StatefulWidget {
  const GMCheckbox(
      {this.id,
      Key? key,
      this.title,
      ...
      this.selectColor,
      this.disableColor,
      this.size = GMCheckBoxSize.small,
      this.cardMode = false,
      this.showDivider = true,
      this.contentDirection = GMContentDirection.right,
  ...
  @override
  State createState() => GMCheckboxState();

  /// 默认的checkBox icon
  Widget buildDefaultIcon(BuildContext context, GMCheckboxGroupState? groupState, bool isChecked) {
    if (cardMode == true) {
      return Container();
    }
    Widget current;
    var size = 24.0;
    final style = this.style ?? groupState?.widget.style ?? GMCheckboxStyle.circle;
    final theme = GMTheme.of(context);
    final deSelectedColor = style == GMCheckboxStyle.check ? Colors.transparent : theme.grayColor4;
    current = Icon(
      style == GMCheckboxStyle.circle
          ? isChecked
              ? GMIcons.check_circle_filled
              : GMIcons.circle
          : style == GMCheckboxStyle.square
              ? isChecked
                  ? GMIcons.check_rectangle_filled
                  : GMIcons.rectangle
              : isChecked
                  ? GMIcons.check
                  : GMIcons.check,
...
class GMCheckboxState extends State<GMCheckbox> {
  bool checked = false;
  bool _pressed = false;
  ...
  @override
  void didUpdateWidget(GMCheckbox oldWidget) {
    checked = widget.checked;
    super.didUpdateWidget(oldWidget);
  }

  double _spacing(GMCheckboxGroupState? groupState) {
    return widget.spacing ?? groupState?.widget.spacing ?? 8;
  }

  EdgeInsets _getPadding(GMCheckBoxSize size) {
    if (widget.cardMode) {
      return const EdgeInsets.only(top: 16);
    }
    switch (size) {
      case GMCheckBoxSize.small:
        return const EdgeInsets.only(top: 12, bottom: 12);
      case GMCheckBoxSize.large:
                    ...
                    children: [
                      ...
                      Visibility(
                        ...
                        child: Padding(
                          ...
                          child: GMText(widget.subTitle ?? '',
                              maxLines: widget.subTitleMaxLine,
                              overflow: TextOverflow.ellipsis,
                              textColor: widget.enable
                                  ? (widget.subTitleColor ?? GMTheme.of(context).fontGyColor3)
                        ...
                        child: Padding(
                          ...
                          child: GMText(widget.subTitle ?? '',
                              ...
                              textColor: widget.enable
                                  ? (widget.subTitleColor ?? GMTheme.of(context).fontGyColor3)
                                  : GMTheme.of(context).fontGyColor4,
                              font: widget.subTitleFont ?? GMTheme.of(context).fontBodyMedium),
                        ),
                      )
                    ],
                  ),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          border: widget.cardMode
              ? checked
                  ? Border.all(width: 1.5, color: widget.selectColor ?? GMTheme.of(context).brandNormalColor)
  ...
  void onValueChange(
    String? id,
    bool value,
    GMCheckboxGroupState? groupState,
  ) {
    if (!widget.enable) {
      return;
    }
  ...
  Widget? _buildContent(
    ...
    if (content == null) {
      if (title != null || customContent != null && title != null) {
        content = GMText(title,
            maxLines: widget.titleMaxLine ?? groupState?.widget.titleMaxLine,
            overflow: TextOverflow.ellipsis,
            textColor: widget.enable
                ? (widget.titleColor ?? GMTheme.of(context).fontGyColor1)
                : GMTheme.of(context).fontGyColor4,
  ...
  /// 构建icon
  Widget? _buildCheckboxIcon(BuildContext context, GMCheckboxGroupState? groupState, bool isCheck) {
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          CustomPaint(
            painter: RadioCorner(
                length: length, radius: radius, fillColor: selectColor ?? GMTheme.of(context).brandNormalColor),
          ),
          const Positioned(
              top: 3,
              left: 2,
