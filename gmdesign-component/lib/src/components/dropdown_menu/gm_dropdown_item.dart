...
import '../tag/gm_select_tag.dart';
import '../tag/gm_tag_styles.dart';
import 'gm_dropdown_inherited.dart';
import 'gm_dropdown_popup.dart';

typedef GMDropdownItemContentBuilder = Widget Function(
    BuildContext context, _GMDropdownItemState itemState, GMDropdownPopup? popupState);

List<GMDropdownItemOption?> _getSelected(List<GMDropdownItemOption>? options) {
...
/// 下拉菜单内容
class GMDropdownItem<T> extends StatefulWidget {
  ...
  /// 选项数据
  final List<GMDropdownItemOption>? options;

  /// 完全自定义展示内容
  final GMDropdownItemContentBuilder? builder;
  ...
  /// 该item在menu上的宽度，仅在[GMDropdownMenu.isScrollable]为true时有效
  final double? tabBarWidth;

  /// [label]和[arrowIcon]/[GMDropdownMenu.arrowIcon]的对齐方式
  final MainAxisAlignment? tabBarAlign;

  /// 该item在menu上的宽度占比，仅在[GMDropdownMenu.isScrollable]为false时有效
  final int? tabBarFlex;

  static const double operateHeight = 73;

  double? get minContentHeight =>
      multiple == true ? (minHeight != null ? minHeight! + GMDropdownItem.operateHeight : null) : minHeight;
  double? get maxContentHeight =>
      multiple == true ? (maxHeight != null ? maxHeight! + GMDropdownItem.operateHeight : null) : maxHeight;

  @override
  _GMDropdownItemState createState() => _GMDropdownItemState();
...
class _GMDropdownItemState extends State<GMDropdownItem> {
  late GMDropdownPopup popupState;
  late ValueNotifier<GMDropdownMenuDirection> directionListenable;

  @override
  Widget build(BuildContext context) {
    popupState = GMDropdownInherited.of(context)!.popupState;
    directionListenable = GMDropdownInherited.of(context)!.directionListenable;
  ...
  Widget _getCheckboxList() {
    var paddingNum = GMTheme.of(context).spacer16;
    var groupCunck = _groupChunkOptions();
    var maxContentHeight = widget.maxContentHeight != null
        ? widget.maxContentHeight!
        : directionListenable.value == GMDropdownMenuDirection.auto
            ? double.infinity
            : max<double>(popupState.maxContentHeight - GMDropdownItem.operateHeight, 0);
    return Column(
      children: [
        Container(
                    ...
                    children: [
                      groupCunck.length == 1 && entry.key == '__default__'
                          ...
                          : Container(
                              ...
                              color: GMTheme.of(context).whiteColor1,
                              child: GMText(entry.key == '__default__' ? context.resource.other : entry.key),
                            ),
                      Container(
                        padding: EdgeInsets.all(paddingNum),
                        color: GMTheme.of(context).whiteColor1,
                        child: GMCheckboxGroupContainer(
  ...
  Widget? _getCheckboxItem(List<GMDropdownItemOption> cols, int index) {
      ...
      customContentBuilder: (context, checked, content) => Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.all(
            Radius.circular(GMTheme.of(context).radiusDefault),
          ),
        ),
        child: Center(
          child: GMText(
  ...
  Widget _getCheckboxOperate() {
    return Container(
      height: GMDropdownItem.operateHeight,
      ...
      decoration: BoxDecoration(
        ...
        border: Border(
          ...
          bottom: directionListenable.value == GMDropdownMenuDirection.up
              ? BorderSide(
                  color: GMTheme.of(context).grayColor3,
                  width: 1,
                )
      ...
      child: Row(children: [
        ...
        SizedBox(width: GMTheme.of(context).spacer16),
        Expanded(
          child: GMButton(
  ...
  EdgeInsets _getPadding(int length, int index, String direction) {
    var value = length - 1 == index ? 0.0 : GMTheme.of(context).spacer12;
    if (direction == 'bottom') {
  ...
  Map<String, List<List<GMDropdownItemOption>>> _groupChunkOptions() {
    var groupedOptions = widget.options?.groupBy<String>((option) => option.group ?? '__default__') ?? {};
    var groupedChunkOptions = <String, List<List<GMDropdownItemOption>>>{};
  ...
  void _handleSelectChange(selected, {List<GMDropdownItemOption>? options}) {
...
/// 选项数据
class GMDropdownItemOption {
  GMDropdownItemOption({
    required this.value,
    required this.label,
    this.disabled = false,
    this.group,
