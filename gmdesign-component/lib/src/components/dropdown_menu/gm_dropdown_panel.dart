...
class GMDropdownPanel extends StatefulWidget {
  const GMDropdownPanel({
    Key? key,
  ...
  final ValueNotifier<GMDropdownPopupDirection> directionListenable;
  ...
  @override
  _GMDropdownPanelState createState() => _GMDropdownPanelState();
}

class _GMDropdownPanelState extends State<GMDropdownPanel> with SingleTickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(GMDropdownPanel oldWidget) {
  ...
  void open(BuildContext itemContext) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      if (widget.directionListenable.value == GMDropdownPopupDirection.auto) {
        // 比较展开方向（down）的高度能不能放下item，能将方向更新为down
        // 否则比较反方向（up）的高度是否大于down的方向，大于则将方向更新为up，否则保持为down
        if (widget.direction == GMDropdownPopupDirection.down) {
          if (widget.initContentBottom >= size.height) {
            widget.directionListenable.value = GMDropdownPopupDirection.down;
          } else {
            if (widget.reverseHeight > widget.initContentBottom) {
              widget.directionListenable.value = GMDropdownPopupDirection.up;
            } else {
              widget.directionListenable.value = GMDropdownPopupDirection.down;
            }
          }
