...
/// 替换：$1$2: this.$1$2,
///
class GMText extends StatelessWidget {
  const GMText(
    this.data, {
    this.font,
    this.fontWeight,
  ...
  Widget build(BuildContext context) {
    ...
    if (forceVerticalCenter && kTextForceVerticalCenterEnable) {
      ...
      var textFont = font ?? GMTheme.of(context).fontBodyLarge ?? Font(size: 16, lineHeight: 24);
      var fontSize = style?.fontSize ?? textFont.size;
      var height = style?.height ?? textFont.height;

      paddingConfig ??= GMTextPaddingConfig.getDefaultConfig();
  ...
  /// 提取成方法，允许业务定义自己的GMTextConfiguration
  GMTextConfiguration? getConfiguration(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<GMTextConfiguration>();
  }
...
class GMTextSpan extends TextSpan {
  /// 构造参数，扩展参数释义可参考[GMText]中字段注释
  GMTextSpan({
...
/// 存储可以自定义GMText居中算法数据的内部控件
class GMTextConfiguration extends InheritedWidget {
  /// forceVerticalCenter=true时，内置padding配置
  final GMTextPaddingConfig? paddingConfig;

  /// 全局字体,kTextNeedGlobalFontFamily=true时生效
  final FontFamily? globalFontFamily;

  const GMTextConfiguration({Key? key, required Widget child, this.paddingConfig, this.globalFontFamily})
      : super(key: key, child: child);

  @override
  bool updateShouldNotify(covariant GMTextConfiguration oldWidget) {
    return paddingConfig != oldWidget.paddingConfig;
  }
}

/// 通过Padding自定义GMText居中算法
class GMTextPaddingConfig {
  static GMTextPaddingConfig? _defaultConfig;
  static final Map _cacheMap = {};

  /// 获取默认配置
  static GMTextPaddingConfig getDefaultConfig() {
    _defaultConfig ??= GMTextPaddingConfig();
