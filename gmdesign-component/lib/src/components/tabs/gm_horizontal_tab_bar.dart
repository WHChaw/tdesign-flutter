...
class _TabStyle extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final defaultStyle = (labelStyle ??
        ...
        TextStyle(
            height: GMTheme.of(context).fontBodyMedium?.height ?? 1.57,
...
class GMHorizontalTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// and the length of the [GMHorizontalTabBarView.children] list.
  final List<GMTab> tabs;
  ...
  /// and the entire [GMHorizontalTabBar] is scrollable. Otherwise each tab gets an equal
  ...
  /// [GMHorizontalTabBar] in a [Padding] widget. When [isScrollable] is true, the scrollable itself is inset,
  ...
  /// An optional callback that's called when the [GMHorizontalTabBar] is tapped.
  ///
  /// The callback is applied to the index of the tab where the tap occurred.
  ///
  /// This callback has no effect on the default handling of taps. It's for
  /// applications that want to do a little extra work when a tab is tapped,
  /// even if the tap doesn't change the TabController's index. GMHorizontalTabBar [onTap]
  /// callbacks should not make changes to the TabController since that would
  /// interfere with the default tap handler.
  final ValueChanged<int>? onTap;

  /// How the [GMHorizontalTabBar]'s scroll view should respond to user input.
  ///
  ...
  /// 选项卡样式
  final GMTabBarOutlineType? outlineType;
  ...
  /// Returns whether the [GMHorizontalTabBar] contains a tab with both text and icon.
  ///
  /// [GMHorizontalTabBar] uses this to give uniform padding to all tabs in cases where
  ...
  @override
  State<GMHorizontalTabBar> createState() => _GMHorizontalTabBarState();
}
...
class _GMHorizontalTabBarState extends State<GMHorizontalTabBar> {
  ...
  // If the GMHorizontalTabBar is rebuilt with a new tab controller, the caller should
  ...
  @override
  void didUpdateWidget(GMHorizontalTabBar oldWidget) {
  ...
  void _handleTabControllerAnimationTick() {
    ...
    if (!_controller!.indexIsChanging && widget.isScrollable) {
      // Sync the GMHorizontalTabBar's scroll position with the GMHorizontalTabBarView's PageView.
      _currentIndex = _controller!.index;
  ...
  BoxDecoration? _getContentDecorateInner(int index) {
    if (widget.outlineType == GMTabBarOutlineType.capsule) {
      return BoxDecoration(
          color: index == _currentIndex
              ? (widget.selectedBgColor ?? GMTheme.of(context).brandColor1)
              : (widget.unSelectedBgColor ?? GMTheme.of(context).grayColor1),
  ...
  BoxDecoration? _getContentDecorateOuter(int index) {
    ...
    } else if (widget.outlineType == GMTabBarOutlineType.card) {
      if (index == _currentIndex) {
        return BoxDecoration(
            color: widget.backgroundColor ?? GMTheme.of(context).whiteColor1,
  ...
  Color? _getBackgroundColor(int index) {
    if (widget.outlineType == GMTabBarOutlineType.card) {
      if (index == _currentIndex) {
        return GMTheme.of(context).grayColor1;
      }
    }
    return null;
  }
  ...
  Widget build(BuildContext context) {
      ...
      if (_controller!.length != widget.tabs.length) {
        throw FlutterError(
          "Controller's length property (${_controller!.length}) does not match the "
              "number of tabs (${widget.tabs.length}) present in GMHorizontalTabBar's tabs property.",
        );
      }
      return true;
    }());
    ...
    if (_controller != null) {
      ...
      if (_controller!.indexIsChanging) {
        ...
        // The user is dragging the GMHorizontalTabBarView's PageView left or right.
      ...
      wrappedTabs[index] = Opacity(
        ...
        child: IgnorePointer(
          ...
          child: InkWell(
            ...
            child: Container(
              padding: widget.outlineType == GMTabBarOutlineType.filled
...
class _TabBarScrollController extends ScrollController {
  ...
  final _GMHorizontalTabBarState tabBar;
...
/// selected tab.
///
/// This widget is typically used in conjunction with a [GMHorizontalTabBar].
///
...
/// [children] list and the length of the [GMHorizontalTabBar.tabs] list.
///
/// To see a sample implementation, visit the [TabController] documentation.
class GMHorizontalTabBarView extends StatefulWidget {
  /// Creates a page view with one child per tab.
  ///
  /// The length of [children] must be the same as the [controller]'s length.
  const GMHorizontalTabBarView({
    Key? key,
  ...
  /// One widget per tab.
  ///
  /// Its length must match the length of the [GMHorizontalTabBar.tabs]
  ...
  @override
  State<GMHorizontalTabBarView> createState() => _GMHorizontalTabBarViewState();
}

class _GMHorizontalTabBarViewState extends State<GMHorizontalTabBarView> {
  TabController? _controller;
  ...
  // If the GMHorizontalTabBarView is rebuilt with a new tab controller, the caller should
  // dispose the old one. In that case the old controller's animation will be
  // null and should not be accessed.
  bool get _controllerIsValid => _controller?.animation != null;
  ...
  @override
  void didUpdateWidget(GMHorizontalTabBarView oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.controller != oldWidget.controller) {
      _updateTabController();
      _currentIndex = _controller!.index;
  ...
  Widget build(BuildContext context) {
    assert(() {
      if (_controller!.length != widget.children.length) {
        throw FlutterError(
          "Controller's length property (${_controller!.length}) does not match the "
              "number of tabs (${widget.children.length}) present in GMHorizontalTabBar's tabs property.",
        );
      }
      return true;
    }());
...
class TabPageSelector extends StatelessWidget {
  ...
  Widget _buildTabIndicator(
    ...
    if (tabController.indexIsChanging) {
      ...
      // The selection's offset reflects how far the GMHorizontalTabBarView has / been dragged
