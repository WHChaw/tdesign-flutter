...
class GMCascaderPage extends StatefulWidget {
  const GMCascaderPage({super.key});

  @override
  State<GMCascaderPage> createState() => _GMCascaderPageState();
}

class _GMCascaderPageState extends State<GMCascaderPage> {
  String? _initData;
  String _selected_1 = '';
  List<Map> _data = [
    {
  ...
  Widget _buildVerticalCascader(BuildContext context) {
    return GestureDetector(
      onTap: () {
        GMCascader.showMultiCascader(context, title: '选择地址', data: _data, initialData: _initData, theme: 'step',
  ...
  Widget _buildVerticalLetterCascader(BuildContext context) {
    return GestureDetector(
      onTap: () {
        GMCascader.showMultiCascader(context, title: '选择地址', data: _data_2, initialData: _initData_2, theme: 'step',
            onChange: (List<MultiCascaderListModel> selectData) {
          setState(() {
            List result = [];
            int len = selectData.length;
  ...
  Widget _buildHorizontalCascader(BuildContext context) {
    return GestureDetector(
      onTap: () {
        GMCascader.showMultiCascader(context,
            title: '选择地址',
  ...
  Widget _buildHorizontalCompanyCascader(BuildContext context) {
    return GestureDetector(
      onTap: () {
        GMCascader.showMultiCascader(context, title: '选择部门人员', data: _data_3,isLetterSort: true, initialData: _initData_3, theme: 'tab',
            onChange: (List<MultiCascaderListModel> selectData) {
          setState(() {
            List result = [];
            int len = selectData.length;
  ...
  Widget _buildVerticalCompanyCascader(BuildContext context) {
    return GestureDetector(
      onTap: () {
        GMCascader.showMultiCascader(context, title: '选择部门人员', data: _data_3,isLetterSort: true, initialData: _initData_3, theme: 'step',
            onChange: (List<MultiCascaderListModel> selectData) {
          setState(() {
            List result = [];
            int len = selectData.length;
                ...
                child: Row(
                  children: [
                    Expanded(
                      ...
                      output,
                      font: GMTheme.of(context).fontBodyLarge,
                      textColor: GMTheme.of(context).fontGyColor3.withOpacity(0.4),
                      maxLines: 1,
                    ...
                    Padding(
                      ...
                      child: Icon(
                        GMIcons.chevron_right,
                        color: GMTheme.of(context).fontGyColor3.withOpacity(0.4),
                      ),
                    ),
                  ],
                ),
