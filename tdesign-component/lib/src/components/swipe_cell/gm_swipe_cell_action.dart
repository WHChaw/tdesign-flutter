...
/// 滑动单元格操作按钮
class GMSwipeCellAction extends StatelessWidget {
  const GMSwipeCellAction({
    Key? key,
    this.flex = 1,
  ...
  /// 宽度占比，默认为 1，[GMSwipeCellPanel.confirms]下无效（失踪占满整个[GMSwipeCellPanel]宽度）
  final int? flex;
  ...
  /// 指定[GMSwipeCellPanel.children]的索引，来打开该[GMSwipeCellAction]
  /// [GMSwipeCellPanel.confirms]参数下才配置该参数
  ...
  Widget build(BuildContext context) {
    final fontSize = GMTheme.of(context).fontMarkMedium ?? Font(size: 14, lineHeight: 22, fontWeight: FontWeight.w600);
    final children = <Widget>[
      if (icon != null)
        Icon(
          icon,
          size: iconSize ?? 18,
          color: labelStyle?.color ?? GMTheme.of(context).fontWhColor1,
        ),
  ...
  void _handleTap(BuildContext context) {
    final swipeInherited = GMSwipeCellInherited.of(context)!;
