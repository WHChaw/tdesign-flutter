...
export 'gm_calendar_body.dart';
export 'gm_calendar_cell.dart';
export 'gm_calendar_header.dart';
export 'gm_calendar_popup.dart';
export 'gm_calendar_style.dart';

typedef CalendarFormat = GMDate? Function(GMDate? day);
...
/// 日历组件
class GMCalendar extends StatefulWidget {
  const GMCalendar({
    Key? key,
  ...
  /// 自定义样式
  final GMCalendarStyle? style;

  /// 选中值变化时触发
  final void Function(List<int> value)? onChange;

  /// 点击日期时触发
  final void Function(int value, DateSelectType type, GMDate tdate)? onCellClick;

  /// 长安日期时触发
  final void Function(int value, DateSelectType type, GMDate tdate)? onCellLongPress;
  ...
  @override
  _GMCalendarState createState() => _GMCalendarState();
}

class _GMCalendarState extends State<GMCalendar> {
  late List<String> weekdayNames;
  late List<String> monthNames;
  late GMCalendarInherited? inherited;
  late GMCalendarStyle _style;
  ...
  void didChangeDependencies() {
    ...
    _style = widget.style ?? GMCalendarStyle.generateStyle(context);
  }

  @override
  void didUpdateWidget(GMCalendar oldWidget) {
  ...
  Widget build(BuildContext context) {
    inherited = GMCalendarInherited.of(context);
    _initValue();
    timePickerModelList.clear();
    final verticalGap = GMTheme.of(context).spacer8;
    return Container(
      ...
      child: Column(
        children: [
          GMCalendarHeader(
            ...
            weekdayGap: GMTheme.of(context).spacer4,
            padding: GMTheme.of(context).spacer16,
          ...
          Expanded(
            child: GMCalendarBody(
              type: widget.type ?? CalendarType.single,
              firstDayOfWeek: widget.firstDayOfWeek ?? 0,
              maxDate: widget.maxDate,
              minDate: widget.minDate,
              value: widget._value,
              bodyPadding: GMTheme.of(context).spacer16,
