...
/// 支持样式：方形/圆角/半圆/带关闭图标
///
class GMTag extends StatelessWidget {
  const GMTag(this.text,
      {this.theme,
      this.icon,
      this.iconWidget,
      this.textColor,
  ...
  Widget? getIcon(GMTagStyle innerStyle){
    if(iconWidget != null){
      return iconWidget;
    }
    if(icon != null){
  ...
  GMTagStyle _getInnerStyle(BuildContext context) {
    if (style != null) {
      return style!;
    }
    if(disable){
      return GMTagStyle.generateDisableSelectStyle(context, isOutline, shape);
    }
    return isOutline
  ...
  Font? _getFont(BuildContext context) {
    switch (size) {
      case GMTagSize.extraLarge:
        return GMTheme.of(context).fontBodyMedium;
      case GMTagSize.large:
        return GMTheme.of(context).fontBodyMedium;
      case GMTagSize.small:
        return GMTheme.of(context).fontBodyExtraSmall;
      default:
        return GMTheme.of(context).fontBodySmall;
    }
  }
