...
import 'gm_rate_overlay.dart';
import 'gm_rate_tips.dart';
...
/// 评分组件
class GMRate extends StatefulWidget {
  const GMRate({
    super.key,
  ...
  /// 评分图标的颜色，示例：[选中颜色] / [选中颜色，未选中颜色]，默认：[GMTheme.of(context).warningColor5, GMTheme.of(context).grayColor4]
  ...
  /// 评分图标的间距，默认：GMTheme.of(context).spacer8
  final double? gap;

  /// 自定义评分图标，[选中和未选中图标] / [选中图标，未选中图标]，默认：[GMIcons.star_filled]
  ...
  /// 评分图标与辅助文字的间距，默认：[GMTheme.of(context).spacer16]
  final double? iconTextGap;

  @override
  _GMRateState createState() => _GMRateState();
}

class _GMRateState extends State<GMRate> with TickerProviderStateMixin {
  /// 节流
  ...
  void initState() {
    ...
    _overlay = GMRateOverlay(context: context, builder: (context) => _buildOverlay())..show();
  ...
  @override
  void didUpdateWidget(GMRate oldWidget) {
        ...
        GestureDetector(
          ...
          child: Row(
            ...
            children: List.generate(widget.count ?? 5, (index) {
              ...
              return Padding(
                padding: EdgeInsets.only(right: isLast ? 0 : widget.gap ?? GMTheme.of(context).spacer8),
  ...
  Widget _getDefText() {
    ...
    return Padding(
      padding: widget.direction == Axis.horizontal
          ? EdgeInsets.only(left: widget.iconTextGap ?? GMTheme.of(context).spacer16)
          : EdgeInsets.only(top: widget.iconTextGap ?? GMTheme.of(context).spacer8),
      child: SizedBox(
        width: widget.textWidth ?? 50,
        child: GMText(
          notRated ? context.resource.notRated : widget.texts?.getOrNull(textIndex.toInt()) ?? '$_activeValue',
          font: notRated ? GMTheme.of(context).fontBodyLarge : GMTheme.of(context).fontTitleMedium,
          textColor: notRated ? GMTheme.of(context).fontGyColor4 : GMTheme.of(context).fontGyColor1,
        ),
      ),
    );
  }
  ...
  Color _getIconColor({double? value, bool? isActive}) {
    return (value != null && _activeValue >= value) || (isActive != null && isActive)
        ? widget.color?.getOrNull(0) ?? GMTheme.of(context).warningColor5
        : widget.color?.getOrNull(1) ?? GMTheme.of(context).grayColor4;
  }

  IconData _getIcon({double? value, bool? isActive}) {
    final selectIcon = widget.icon?.getOrNull(0) ?? GMIcons.star_filled;
  ...
  Widget _buildOverlay() {
    ...
    return Positioned(
      top: widget.placement == PlacementEnum.top
          ? rateOffset.dy - GMTheme.of(context).spacer8 - _tipSize.height
          : rateOffset.dy + GMTheme.of(context).spacer8 + rateSize.height,
