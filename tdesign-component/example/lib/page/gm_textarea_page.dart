...
class GMTextareaPage extends StatefulWidget {
  const GMTextareaPage({Key? key}) : super(key: key);

  @override
  _GMTextareaPageState createState() => _GMTextareaPageState();
}

class _GMTextareaPageState extends State<GMTextareaPage> {
  ...
  Widget _cardStyle(BuildContext context) {
    return GMTextarea(
      ...
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(GMTheme.of(context).radiusExtraLarge),
      ),
      margin: EdgeInsets.only(right: GMTheme.of(context).spacer16, left: GMTheme.of(context).spacer16),
      onChanged: (value) {
        setState(() {});
      },
    );
